...
function mock(BigscreenPlayer, opts) {
  ...
  if (mockStatus.currentlyMocked) {
    throw new Error(
      "mock() was called while BigscreenPlayer was already mocked"
    );
  }
...
function mockJasmine(BigscreenPlayer, opts) {
  ...
  if (mockStatus.currentlyMocked) {
    throw new Error(
      "mockJasmine() was called while BigscreenPlayer was already mocked"
    );
  }
...
var mockFunctions = {
  init: function(
    ...
    currentTime =
      (bigscreenPlayerData && bigscreenPlayerData.initialPlaybackTime) || 0;
    liveWindowStart = undefined;
    pausedState = true;
    endedState = false;
    mediaKind =
      (bigscreenPlayerData &&
        bigscreenPlayerData.media &&
        bigscreenPlayerData.media.kind) ||
      "video";
  ...
  tearDown: function() {
    ...
    Plugins.interface.onBufferingCleared(
      new PluginData({
        ...
        properties: { dismissed_by: "teardown" },
...
var mockingHooks = {
  changeState: function(state, eventTrigger, opts) {
    ...
    var pauseTrigger =
      opts && opts.userPause === false ? PauseTriggers.APP : PauseTriggers.USER;

    pausedState =
      state === MediaState.PAUSED ||
      state === MediaState.STOPPED ||
      state === MediaState.ENDED;
    ...
    if (state === MediaState.WAITING) {
      ...
      Plugins.interface.onBufferingCleared(
        new PluginData({
          status: PluginEnums.STATUS.DISMISSED,
          stateType: PluginEnums.TYPE.BUFFERING,
          properties: { dismissed_by: eventTrigger },
          isInitialPlay: initialBuffering
        })
      );
    }
    ...
    if (state === MediaState.FATAL_ERROR) {
      ...
      stateObject.isBufferingTimeoutError =
        opts && opts.isBufferingTimeoutError;
    }
  ...
  triggerErrorHandled: function() {
    ...
    Plugins.interface.onBufferingCleared(
      new PluginData({
        status: PluginEnums.STATUS.DISMISSED,
        stateType: PluginEnums.TYPE.BUFFERING,
        properties: { dismissed_by: "timeout" },
        isInitialPlay: initialBuffering
      })
    );
    Plugins.interface.onErrorCleared(
