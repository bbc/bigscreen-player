...
describe("IMSC Subtitles", () => {
  ...
  beforeEach(() => {
    ...
    mockMediaSources.currentSubtitlesSource.mockImplementation(() => subtitlesUrl)
    ...
    mockMediaSources.currentSubtitlesSegmentLength.mockImplementation(() => segmentLength)
    mockMediaSources.currentSubtitlesCdn.mockImplementation(() => subtitlesCdn)
  ...
  describe("Live subtitles", () => {
    ...
    describe("rendering", () => {
      ...
      it("should generate and render when time has progressed past a known un-rendered subtitles", () => {
        ...
        timeSegments = timeSegments.map((times) =>
          times.map((time) => (time === 0 ? time : time + msToS(epochStartTimeMilliseconds)))
        )
        ...
        fromXML.mockImplementation(() => {
          ...
          return {
            getMediaTimeEvents: () => timeSegments[counter],
            mockCallId: counter,
            head: {
              styling: {},
            },
