...
describe("IMSC Subtitles", () => {
  ...
  const mockFromXmlDefaultReturnValue = {
  ...
  fromXML.mockReturnValue(mockFromXmlDefaultReturnValue)
  ...
  describe("construction", () => {
    ...
    it("starts the update loop immediately when set to autoplay", () => {
      ...
      expect(generateISD).toHaveBeenCalledWith(mockFromXmlDefaultReturnValue, 1.5)
  ...
  describe("subtitles delivered as a whole", () => {
    ...
    it("attempts to generate and render when the initial current time is greater than the final subtitle time", () => {
      ...
      expect(generateISD).toHaveBeenCalledTimes(1)
      expect(generateISD).toHaveBeenCalledWith(mockFromXmlDefaultReturnValue, 9)
      expect(renderHTML).toHaveBeenCalledTimes(1)
    ...
    it("attempts to generate and render when the initial current time is mid way through a stream", () => {
      ...
      expect(generateISD).toHaveBeenCalledTimes(1)
      expect(generateISD).toHaveBeenCalledWith(mockFromXmlDefaultReturnValue, 4)
      expect(renderHTML).toHaveBeenCalledTimes(1)
    })
    ...
    it("only generates and renders when there are new subtitles to display", () => {
      ...
      expect(generateISD).toHaveBeenCalledWith(mockFromXmlDefaultReturnValue, 3)
  ...
  describe("Live subtitles", () => {
    ...
    describe("Loading segments", () => {
      it("fills the buffer with correct segment urls on the first update interval", () => {
        ...
        progressTime(0.1) // currentTime > windowStartEpoch
        // jest.advanceTimersByTime(UPDATE_INTERVAL)
    ...
    describe.skip("rendering", () => {
      it("generates and renders when time has progressed past a known unrendered subtitles segment", () => {
        ...
        let timeEventsForSegments = [
          [0, 1, 2, 3.84],
          [0, 3.84, 4, 7.68],
          [0, 7.68, 9, 9.7, 11.52],
        ]

        timeEventsForSegments = timeEventsForSegments.map((segmentTimes) =>
          segmentTimes.map((time) => (time === 0 ? 0 : epochStartTimeSeconds + time))
        )

        let segmentsConsumedSoFar = 0

        fromXML.mockImplementation(() => {
          const imscDocument = {
            _mockedCallsSoFar: segmentsConsumedSoFar,
            getMediaTimeEvents: () => timeEventsForSegments[segmentsConsumedSoFar],
            head: {
              styling: {},
            },
            body: {
              contents: Array.from({ length: 1 }),
            },
          }

          segmentsConsumedSoFar += 1

          return imscDocument
        })

        generateISD.mockReturnValueOnce({ contents: ["mockContents"] })
        ...
        expect(generateISD).toHaveBeenCalledWith(
          expect.objectContaining({ _mockedCallsSoFar: 0 }),
