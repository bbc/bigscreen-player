function filter(manifest, representationOptions) {
  ...
  if (constantFps || maxFps) {
    manifest.Period.AdaptationSet = manifest.Period.AdaptationSet.map(function(
      adaptationSet
    ) {
      if (adaptationSet.contentType === "video") {
        var frameRates = [];

        adaptationSet.Representation_asArray = adaptationSet.Representation_asArray.filter(
          ...
          return (
            !constantFps ||
            representation.frameRate === Math.max.apply(null, frameRates)
          );
        });
      }
