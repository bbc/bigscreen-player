...
function parseMPD(manifest, dateWithOffset) {
  try {
    ...
    if (availabilityStartTime && oneSegment) {
      var windowEndTime =
        dateWithOffset -
        (timeShiftBufferDepth ? availabilityStartTime : 0) -
        oneSegment;
      var windowStartTime = timeShiftBufferDepth
        ? windowEndTime - timeShiftBufferDepth * 1000
