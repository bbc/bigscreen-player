...
function BigscreenPlayer() {
  ...
  function mediaStateUpdateCallback(evt) {
    ...
    if (evt.data.seekableRange) {
      DebugTool.keyValue({
        key: "seekableRangeStart",
        value: deviceTimeToDate(evt.data.seekableRange.start)
      });
      DebugTool.keyValue({
        key: "seekableRangeEnd",
        value: deviceTimeToDate(evt.data.seekableRange.end)
      });
    }
  ...
  function convertVideoTimeSecondsToEpochMs(seconds) {
    return getWindowStartTime()
      ? getWindowStartTime() + seconds * 1000
      : undefined;
  }

  function bigscreenPlayerDataLoaded(
    playbackElement,
    bigscreenPlayerData,
    enableSubtitles,
    device,
    ...
    DebugTool.keyValue({
      key: "current cdn",
      value: bigscreenPlayerData.media.urls[0].cdn
    });
    DebugTool.keyValue({
      key: "url",
      value: bigscreenPlayerData.media.urls[0].url
    });
  }
  ...
  return {
    ...
    setCurrentTime: function(time) {
      ...
      if (playerComponent) {
        ...
        endOfStream =
          windowType !== WindowTypes.STATIC &&
          Math.abs(this.getSeekableRange().end - time) <
            END_OF_STREAM_TOLERANCE;
      }
    },
    ...
    convertEpochMsToVideoTimeSeconds: function(epochTime) {
      return getWindowStartTime()
        ? Math.floor((epochTime - getWindowStartTime()) / 1000)
        : undefined;
    },
