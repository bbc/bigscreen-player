...
export default function(
  ...
  var strategy =
    window.bigscreenPlayer && window.bigscreenPlayer.playbackStrategy;
  ...
  var setSourceOpts = {
    disableSentinels:
      ...
      config.streaming &&
      config.streaming.liveUhdDisableSentinels
  };
  ...
  function setupExitSeekWorkarounds(mimeType) {
    handleErrorOnExitingSeek =
      windowType !== WindowTypes.STATIC && mimeType === "application/dash+xml";

    var capabilities = config.capabilities || [];
    var deviceFailsPlayAfterPauseOnExitSeek =
      capabilities.indexOf("playFailsAfterPauseOnExitSeek") !== -1;
    delayPauseOnExitSeek =
      handleErrorOnExitingSeek || deviceFailsPlayAfterPauseOnExitSeek;
  }
  ...
  function checkSeekSucceeded(seekableRangeStart, currentTime) {
    ...
    var successfullySeeked =
      Math.abs(currentTime - clampedSeekToTime) < SEEK_TOLERANCE;
  ...
  function showCurtain() {
    ...
    var shouldShowCurtain =
      windowType !== WindowTypes.STATIC &&
      (hasStartTime || overrides.forceBeginPlaybackToEndOfWindow);
  ...
  return {
    ...
    load: function(cdns, mimeType, startTime) {
      ...
      var isPlaybackFromLivePoint =
        windowType !== WindowTypes.STATIC && !hasStartTime;
    ...
    getSeekableRange: function() {
      if (windowType === WindowTypes.STATIC) {
        ...
        var seekableRange =
          (mediaPlayer.getSeekableRange && mediaPlayer.getSeekableRange()) ||
          {};
