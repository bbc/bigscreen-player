...
function Player(logger) {
  ...
  function enterBufferingSentinel() {
    var sentinelShouldFire =
      !hasSentinelTimeChangedWithinTolerance && !nearEndOfMedia;
  ...
  function shouldBeSeekedSentinel() {
    ...
    if (Math.abs(currentTime - sentinelSeekTime) > seekSentinelTolerance) {
      sentinelActionTaken = nextSentinelAttempt(
        sentinelLimits.seek,
        function() {
  ...
  function setSentinels(sentinels) {
    ...
    sentinelInterval = setInterval(function() {
      ...
      hasSentinelTimeChangedWithinTolerance =
        Math.abs(newTime - lastSentinelTime) > 0.2;
  ...
  return {
    ...
    initialiseMedia: function(type, url, mediaMimeType, sourceContainer, opts) {
      ...
      if (getState() === MediaPlayerBase.STATE.EMPTY) {
        var idSuffix = "Video";
        if (
          mediaType === MediaPlayerBase.TYPE.AUDIO ||
          mediaType === MediaPlayerBase.TYPE.LIVE_AUDIO
        ) {
          idSuffix = "Audio";
        }
        ...
        mediaElement = document.createElement(
          idSuffix.toLowerCase(),
          "mediaPlayer" + idSuffix
        );
        ...
        toError(
          "Cannot set source unless in the '" +
            MediaPlayerBase.STATE.EMPTY +
            "' state"
        );
      }
    },
    ...
    beginPlayback: function() {
      ...
      switch (getState()) {
        ...
        default:
          toError(
            "Cannot beginPlayback while in the '" + getState() + "' state"
          );
          break;
      }
    },
    ...
    beginPlaybackFrom: function(seconds) {
      ...
      switch (this.getState()) {
        ...
        default:
          toError(
            "Cannot beginPlaybackFrom while in the '" + getState() + "' state"
          );
          break;
      }
    },
