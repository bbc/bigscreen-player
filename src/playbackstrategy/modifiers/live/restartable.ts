...
function RestartableLivePlayer(deviceConfig, logger) {
  ...
  function detectCurrentTimeCallback(event) {
    if (
      event.state === MediaPlayerBase.STATE.PLAYING &&
      event.currentTime > 0
    ) {
  ...
  function autoResumeAtStartOfRange() {
    if (millisecondsUntilStartOfWindow !== null) {
      var resumeTimeOut = Math.max(
        0,
        millisecondsUntilStartOfWindow -
          AUTO_RESUME_WINDOW_START_CUSHION_MILLISECONDS
      );
  ...
  function determineBufferingCallback(event) {
    if (
      event.state === MediaPlayerBase.STATE.BUFFERING &&
      bufferingStarted === null
    ) {
      ...
      millisecondsUntilStartOfWindow = Math.max(
        0,
        millisecondsUntilStartOfWindow - timeBuffering
      );
      bufferingStarted = null;
    }
  }
