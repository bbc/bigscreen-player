...
var PlayerComponent = (
  ...
  function pause(opts) {
    ...
    if (transitions().canBePaused()) {
      var disableAutoResume =
        windowType === WindowTypes.GROWING ? true : opts.disableAutoResume;
  ...
  function attemptCdnFailover(errorProperties, bufferingTimeoutError) {
    var shouldFailover = MediaResilience.shouldFailover(
      mediaMetaData.urls.length,
      getDuration(),
      getCurrentTime(),
  ...
  function manifestReloadFailover(
    failoverTime,
    thenPause,
    errorProperties,
    bufferingTimeoutError
  ...
  function createPlaybackProperties() {
    ...
    playbackProperties.seekable_range =
  ...
  function initialMediaPlay(media, startTime) {
    ...
    if (!captionsContainer) {
      captionsContainer = new CaptionsContainer(
        playbackStrategy,
        captionsURL,
        isSubtitlesEnabled(),
        playbackElement
