...
function update(logs) {
  view.render({
    static: parseStaticFields(logs),
    dynamic: parseDynamicFields(logs)
  });
}
      ...
      switch (log.type) {
        ...
        case Chronicle.TYPES.TIME:
          return (
            dateString +
            " - Video time: " +
            parseFloat(log.currentTime).toFixed(2)
          );
        case Chronicle.TYPES.EVENT:
          return (
            dateString + " - Event: " + convertToReadableEvent(log.event.state)
          );
