define(
  ...
  function (MediaPlayerBase, MediaElement) {
    'use strict';
    ...
    function Player (deviceConfig) {
      ...
      function shouldBeSeekedSentinel () {
        ...
        if (Math.abs(currentTime - sentinelSeekTime) > seekSentinelTolerance) {
          sentinelActionTaken = nextSentinelAttempt(sentinelLimits.seek, function () {
            mediaElement.setCurrentTime(sentinelSeekTime);
          });
      ...
      function getSeekableRange () {
        if (mediaElement) {
          if (isReadyToPlayFrom() && mediaElement.getSeekable() && mediaElement.getSeekable().length > 0) {
            return {
              start: mediaElement.getSeekable().start(0),
              end: mediaElement.getSeekable().end(0)
            };
          } else if (mediaElement.getDuration() !== undefined) {
            return {
              start: 0,
              end: mediaElement.getDuration()
            };
          }
        }
        return undefined;
      ...
      function onError () {
        reportError('Media element error code: ' + mediaElement.getErrorCode());
      }
      ...
      function seekTo (seconds) {
        ...
        mediaElement.setCurrentTime(clampedTime);
      ...
      */
      ...
      return {
        ...
        getPlayerElement: function () {
          return mediaElement.getPlayerElement();
        },
